...
//---------CHEMIN
define("RACINE_SITE", $_SERVER['DOCUMENT_ROOT'] . '/Back/WF3/PHP/evals/eval_php_sylvain_baillet.php/' ); //lors de l'enregistrement d'image / photos, nous aurons besoin du chemin physique pour enregistrer la photo dans le bon dossier 
// echo RACINE_SITE; 


define("URL", "http://localhost/Back/WF3/PHP/evals/eval_php_sylvain_baillet.php/");/* Cette constante servira a enregistrer  l'URL d'une photo / image dans la BDD. On ne conserve jamais la photo elle même, ce serait trop lourd pour la BDD */
        ...
        if(empty($errorTitre) && empty($errorAddr) && empty($errorVille) && empty($errorCp) && empty($errorSurf) && empty($errorPrix) && empty($errorType))   
            {
                ...
                $nouvLogement = $bdd->prepare("INSERT INTO logement (titre , adresse , ville , cp , surface , prix , photo , type , description) VALUES (:titre , :adresse , :ville , :cp , :surface , :prix , :photo , :type , :description )");
                    foreach($_POST as $key => $value)// pour chaque ligne entrée dans le formulaire, on fait un bindvalue
                        ...
                        $nouvLogement->bindValue(":photo", $photo_bdd, PDO::PARAM_STR);
                        }
