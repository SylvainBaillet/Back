...
$(document).ready(function () {
    ...
    function ajax() {
        var prenom = $('#prenom').val(); // on selectionne chaque champs avec son id, ici 'prenom' afin de récuperer la valeur saisie dans le champ grace à la methode 'val()' pour la transmettre a la requete aller AJAX
        var nom = $('#nom').val(); 
        var sexe = $('#sexe').val(); 
        ...
        // pour envoyer plusieurs parametres, on concataine avec + et des le deuxieme parametre on rajoute '&' devant
        var parameters = "prenom=" + prenom + "&nom=" + nom + "&sexe=" + sexe + "&service=" + service + "&date_embauche=" + date_embauche + "&salaire=" + salaire;/* On definie les parametres à envoyer avec la requete 'aller' AJAX. 'personne=' permet de definir ou le parametre va être envoyé dans le fichier PHP, donc dans $personne */
        ...
        $.post("ajax6.php", parameters, function (data) {//post est une requete 
            ...
            'data.resultat' --> resultat correspondant a l'indice 'resultat' du tablau array $tab['resultat'] du fichier ajax6.php*/
            $('#message').html(data.message);
            $('#formulaire')[0].reset();// permet de vider chaque champ du formulaire apres la validation, le '[0]' corrersponde à l'indice auquel se trouve le formulaire, si il y'a plusieurs formulaires dans la page, il faudra crocheter au bon indice.
            //autre methode:
