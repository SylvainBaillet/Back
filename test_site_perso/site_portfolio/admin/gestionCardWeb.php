...
if($_POST)
    {
        ...
        if(isset($_GET['action']) && $_GET['action'] == 'modification'){
            ...
            {
            $nom_photo = $photoweb . '-' . $_FILES['photo']['name'];/* on redefinie le nom de la photo en concaténant la référence saisie dans le formulaire avec le nom de la photo */
            // echo $nom_photo . '<br>';

            $photo_bdd = URL . "images/$photoweb";//URL est une constante, tout comme RACINE_SITE
            // echo $photo_bdd . '<br>';

            $photo_dossier = RACINE_SITE . "images/$photoweb";/* on définie l'URL de la photo, c'est ce que l'on conservera en BDD */
        ...
        if(isset($_GET['action']) && $_GET['action'] == 'ajout')
            {
                ...
                $validate .= "<div class='alert alert-success col-md-6 offset-md-3 text-center'>La photo : <strong>$title</strong> à bien été ajoutée </div>";
            }
        else  
            {
                $addPhoto = $bdd->prepare("UPDATE cardweb SET photoweb = :photoweb, titre = :titre WHERE id_cardweb = $id_cardweb");

                $validate .= "<div class='alert alert-success col-md-6 offset-md-3 text-center'>La photo : <strong>$title</strong> à bien été modifiée </div>";
            }
...
<button type="submit" class="btn btn-primary boutonAjout offset-md-3 mb-2"><a href="?action=ajout#formulaire" class="text text-light">Ajout Card</a></button>
